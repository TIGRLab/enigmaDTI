...
mkdir ENIGMA_HILLSIDE_ALL
dirO1=./ENIGMA_HILLSIDE_ALL/

for subject in $( ls ../ALL | grep vol )

do
base=$(basename $subject .nii.gz);
echo "Basename $base"
./singleSubjROI_exe ENIGMA_look_up_table.txt mean_FA_skeleton_mask.nii.gz JHU-WhiteMatter-labels-1mm.nii.gz ${dirO1}${base}_ROIout ../ALL/${subject}
...
#make an output directory for all files
mkdir ENIGMA_HILLSIDE_ALL_2
dirO2=./ENIGMA_HILLSIDE_ALL_2/
...
for subject in $( ls ../ALL/ | grep vol | grep .nii.gz)
