...
mkdir ENIGMA_ROI_STEP_FA3T
dirO1=./ENIGMA_ROI_STEP_FA3T/

for subject in $( ls ../FNIRT_FA/FA/ | grep FA_to_target.nii.gz)

do
base=$(basename $subject _FA_FA_to_target.nii.gz);
echo "Basename $base"
./singleSubjROI_exe ENIGMA_look_up_table.txt mean_FA_skeleton_mask.nii.gz JHU-WhiteMatter-labels-1mm.nii.gz ${dirO1}${base}_ROIout ../FNIRT_FA/FA/${subject}
...
#make an output directory for all files
mkdir ENIGMA_ROI_STEP2_FA3T
dirO2=./ENIGMA_ROI_STEP2_FA3T/
...
do
base=$(basename $subject _FA_to_target.nii.gz);
