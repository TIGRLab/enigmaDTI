...
for subject in $( ls  /projects/david/3T_DIST_CORR/REDUX/DISPERSION_CURVATURE/TBSS/1.5T/FA/  | grep FA_to_target_DISP.nii.gz)

do
base=$(basename $subject _FA_FA_to_target_DISP.nii.gz);
echo "Basename $base"
./singleSubjROI_exe ENIGMA_look_up_table.txt mean_FA_skeleton_mask.nii.gz JHU-WhiteMatter-labels-1mm.nii.gz ${dirO1}${base}_ROIout /projects/david/3T_DIST_CORR/REDUX/DISPERSION_CURVATURE/TBSS/1.5T/FA/${subject}
