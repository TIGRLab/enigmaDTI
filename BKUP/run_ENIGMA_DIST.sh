...
for subject in $( ls ../TBSS_FA/FA | grep FA_to_target.nii.gz)

do
base=$(basename $subject _FA_FA_to_target.nii.gz);
echo "Basename $base"
./singleSubjROI_exe ENIGMA_look_up_table.txt mean_FA_skeleton_mask.nii.gz JHU-WhiteMatter-labels-1mm.nii.gz ${dirO1}${base}_ROIout ../TBSS_FA/FA/${subject}
