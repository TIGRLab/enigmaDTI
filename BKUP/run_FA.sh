...
for subject in $( ls ../FA | grep FA_FA_to_target.nii.gz)

do
base=$(basename $subject _FA_FA_to_target.nii.gz);
echo "Basename $base"
./singleSubjROI_exe ENIGMA_look_up_table.txt mean_FA_skeleton_mask.nii.gz JHU-WhiteMatter-labels-1mm.nii.gz ${dirO1}${base}_ROIout ../FA/${subject}
...
#make an output directory for all files
mkdir ENIGMA_3T_FA_2
dirO2=./ENIGMA_3T_FA_2/
