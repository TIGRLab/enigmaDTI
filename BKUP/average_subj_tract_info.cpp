/*
 *  Written by Neda Jahanshad <neda.jahanshad@loni.ucla.edu>
 *  2012 ENIGMA-DTI
 *
 */
...
/*float* wt_avg_roi_values (map<string,float> ROImap_u, map<string,float> ROImap_w,int nROIs, string ROI1,string ROI2, ...)
{
	float* pointer;
	float info[1];
	pointer=info;
	...
	cout << "avgROI " << info[0] << endl;
	cout << "total voxels " << info[1] << endl;
...
void wt_avg_roi_values (map<string,float> ROImap_u, map<string,float> ROImap_w,float &averageROI, float &nvtotal, int nROIs, string ROI1,string ROI2, ...)
{
	...
	averageROI = sumROI/nvtotal;
		...
		//cout << "avgROI " << averageROI << endl;
		//cout << "total voxels " << nvtotal << endl;
}
